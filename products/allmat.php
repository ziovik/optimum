								...
								<table cellspacing="0" id="group-test" class="table table-small-font table-bordered table-striped">
									<thead>
										...
										if (isset($_GET['allmat'])) {
											...
											$get_allmat_pro = "select * from products p 

											join distributors d on p.dist_id = d.dist_id 
											join customers c on d.region_id = c.region_id 
											where c.customer_id = $c_id AND product_cat='$allmat_id'";
											...
											if ($count_allmat == 0) {
												...
												while($row_allmat_pro=mysqli_fetch_array($run_allmat_pro)){
													...
													$pro_cat = $row_allmat_pro['product_cat'];
													$pro_sub_cat = $row_allmat_pro['product_sub_cat'];
													$pro_reg = $row_allmat_pro['region_id'];
													$pro_dist = $row_allmat_pro['dist_id'];
													$pro_name = $row_allmat_pro['product_title'];
													$pro_price = $row_allmat_pro['product_price'];
													$pro_manu = $row_allmat_pro['product_manu'];
													$pro_desc = $row_allmat_pro['product_desc'];
